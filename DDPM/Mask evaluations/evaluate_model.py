#!/usr/bin/env python3
"""
Mask-based CBF Evaluation for Diffusion Baseline Model

This script uses the SAVED predictions (out_*.npy files) rather than re-running
inference, since diffusion sampling is slow (200 steps per sample).

The test set order is deterministic (seed=1337), so we can map indices to patients.
"""
...
BASE_DIR = Path(__file__).parent.parent  # DDPM folder
...
PRED_DIR = BASE_DIR / "runs_diffusion_post_from_pre" / "pred_samples"
...
def get_test_patient_order():
    ...
    This uses the same seed and split logic as diffusion_model.py.
    """
    import re
    ...
    # Get all paired scans (same logic as training)
    ...
    # Extract patient IDs from test set (in order)
    test_patients = []
    ...
    with open(patient_file, 'r') as f:
        for line in f:
            ...
            if len(parts) >= 4:
                ...
                mapping[patient_id] = parts[3]  # mask_side
    return mapping


def load_mask_for_2d(mask_side, masks_dir, target_shape):
    """
    Load the appropriate MCA mask middle slice and resize to target shape.
    """
...
def create_cohort_summary_plot(results_by_side, save_path):
    ...
    plt.suptitle('DDPM: Cohort Summary', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    print("=" * 60)
    print("Diffusion Baseline - Mask-based CBF Evaluation")
    print("=" * 60)
    
    EVAL_DIR.mkdir(exist_ok=True)
    
    # Get the test patient order (matching prediction indices)
    test_patients = get_test_patient_order()
    print(f"\nFound {len(test_patients)} test patients from split")
    ...
    output = {
        'model': 'DDPM',
        'description': 'CBF % change comparison: Ground Truth vs Diffusion Prediction',
        'note': 'Evaluation on middle axial slice (uses saved predictions)',
        'patients': results
    }
