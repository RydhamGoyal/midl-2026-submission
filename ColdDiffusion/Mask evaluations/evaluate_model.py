#!/usr/bin/env python3
"""
Mask-based CBF Evaluation for Cold Diffusion Model

This script uses the SAVED predictions (out_*.npy files).
The test set uses a 15% split with seed=1337.
"""
...
BASE_DIR = Path(__file__).parent.parent  # ColdDiffusion folder
...
PRED_DIR = BASE_DIR / "runs_cold_diffusion" / "pred_samples"
...
def get_test_patient_order():
    ...
    Cold Diffusion uses 25% test split (standardized across all models).
    """
    import re
    ...
    # Same split as Cold Diffusion training: 25% test (standardized)
...
def create_cohort_summary_plot(results_by_side, save_path):
    ...
    plt.suptitle('Cold Diffusion: Cohort Summary', fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    print("=" * 60)
    print("Cold Diffusion - Mask-based CBF Evaluation")
    print("=" * 60)
    ...
    output = {
        'model': 'ColdDiffusion',
        'description': 'CBF % change comparison: Ground Truth vs Cold Diffusion',
    ...
    for side in ['left', 'right', 'both']:
        ...
        print(f"  Cold Diffusion Prediction:")
        if pred_unhealthy:
