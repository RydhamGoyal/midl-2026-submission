...
# IMPORTANT: script is in DDPM/, data is one level up
...
def main():
    ...
    if len(train_pre) > 10:
        tr_pre, val_pre = train_test_split(
            train_pre, test_size=0.12, random_state=SEED, shuffle=True
        )
    else:
        tr_pre = train_pre
        val_pre = train_pre
    ...
    # Save test metrics as JSON as well (nice for tables)
