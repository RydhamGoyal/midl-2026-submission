#!/usr/bin/env python3
"""
Script to identify which patients are in the test set for cAE.
...
SEED = 1337
VAL_FRACTION = 0.25  # Same as model.py
...
def main():
    ...
    # Same split as model.py (25% test via VAL_FRACTION)
    ...
    print("\n" + "=" * 60)
    print("TEST SET PATIENTS (cAE)")
    print("=" * 60)

    test_patients = []
    for pre_path in sorted(test_pre):
        ids = id_from_pre_path(pre_path)
        if ids:
            year, patient_id = ids
