...
# Paths - script is in ColdDiffusion/, data is one level up
...
# Training / Evaluation
# ============================================================
def train_one_epoch(model, loader, optimizer, criterion_l1, criterion_ssim):
    model.train()
    total_loss = 0.0
    n = 0
    ...
    for pre_t, post_t in loader:
        ...
        # We use a single t value for simplicity in forward pass
        # For batch processing, we use the average t or iterate
        x0_preds = []
        for i in range(B):
